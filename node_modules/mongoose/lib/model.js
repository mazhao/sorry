 ...
 *
 * The `fn` callback is optional. If no `fn` is passed and validation fails, the validation error will be emitted on the connection used to create this models.
 *
 *     var db = mongoose.createConnection(..);
 *     var schema = new Schema(..);
 *     var Product = db.models('Product', schema);
 *
 *     db.on('error', handleError);
 *
 * However, if you desire more local error handling you can add an `error` listener to the models and handle errors there instead.
 *
...
/**
 * Called when the models compiles.
 *
 * @api private
 */
 ...
 *
 * If `options.out` is set to `replace`, `merge`, or `reduce`, a Model instance is returned that can be used for further querying. Queries run against this models are all executed with the `lean` option; meaning only the js object is returned and no Mongoose magic is applied (getters, setters, etc).
 *
 * ####Example:
 *
 *     var o = {};
...
function populate (model, docs, options, cb) {
  ...
  // models name for the populate query
...
function populateDocs (docs, options, cb) {
  ...
  for (i = 0; i < len; i++) {
    ...
    if (isDocument) {
      // cache original populated _ids and models used
